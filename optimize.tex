...
struct dag {  // dag の節点
  ...
  dag*         m_left;      // 左の子 x := y op z の y
  dag*         m_right;     // 右の子 x := y op z の z
  dag*         m_extra;     // 3 番目の子 x[y] := z における x
  static vector<dag*> all;  // すべてのリスト
  dag() : m_tac(0), m_left(0), m_right(0), m_extra(0) { all.push_back(this); }
};
...
\item \label{optimize109}
      ...
      \item {\tt{node[z]}} とその節点の左の子が一致している.
      \item {\tt{x}} の型と {\tt{z'}} の型が {\it compatible} である.
      \end{itemize}
      もしあればその右の子を $n$ とします.
\item \label{optimize064}
そのような節点がなければ
      新しい節点を作成し, それを $n$ とします. {\tt{x[y] := z}} に対して,
      {\tt{node[x]}} があるのならば $n$ の 3 番目の子とします.
\end{enumerate}
...
\item \label{optimize048}
      ...
      左の子 $\ell$ に対して, {\tt{result[$\ell$]}} で {\tt{y}} を置き換えます.
\item \label{optimize049}
      節点のラベルを {\tt{x := y $op$ z}} とするとき
      右の子 $r$ に対して, {\tt{result[$r$]}} で {\tt{z}} を置き換えます.
...
\item {\tt{x}} がこの基本ブロックの出口で生きているのならば
      {\tt{x}} を返し, この手続きを終了します.
\item {\tt{x}} がこの後, この基本ブロックで定義される前に使用され,
...
\item 手順 \ref{optimize050} でコピーした 3 番地コードが
      {\tt{alloca x, y}} ならば
      {\tt{x}} を返し, この手続きを終了します.
\item 手順 \ref{optimize050} でコピーした 3 番地コードが {\tt{x[y] := z}} な
      らば {\tt{x}} の節点の識別子並びに
      \begin{enumerate}
      \item この基本ブロックの出口で生きているものがある
      \item この後この基本ブロックで定義される前に使用されるものがある
      \end{enumerate}
      ならば {\tt{x}} を返し, この手続きを終了します.
...
\item  \label{optimize067}
      以下の{\bf 代入判定アルゴリズムにおける特別な場合}に該当す
      るかを調べます. 該当すればその戻り値を返し, この手続を終了します.
\item \label{optimize055}
      この節点が葉ならば識別子並び {\tt{m\_vars}} の先頭を
      {\tt{y}} とします.
\item この節点のラベルが {\tt{loff}} ならば {\tt{m\_vars}} の先頭を
      {\tt{y}} とします.
\item \label{optimize054}
      この節点が葉でなければ識別子並びから以下の手順で 1 つを選択し, 
      それを {\tt{y}} とします.
      \begin{enumerate}
      \item 現在の基本ブロックの出口で生きているもの
      \item このあと定義の前に使用があるもの
      \item この節点が {\tt{node[y]}} と一致するもの
      \item 現在の基本ブロックでアドレス参照されているもの
      \end{enumerate}
      もし該当するものがなければ
...
\item 識別子並びで {\tt{y}} の次のものからすべてに対して
      以下を適用します.
\begin{enumerate}
\item この節点のラベルが {\tt{loff}} ならば
      3 番地コード {\tt{x := y}} を生成します.
\item \label{optimize052}
      識別子 {\tt{x}} に対して, この節点の親のラベルが {\tt{p := \&x}}
      ならば, 節点が葉の場合のみ 3 番地コード {\tt{x := y}} を生成します.
\item この節点の親 {\tt{p}} で以下の条件を満たすものがあるのならば 3 番地コ
      ード {\tt{x := y}} を生成します.
      \begin{enumerate}
      \item {\tt{p}} のラベルが {\tt{loff}} である
      \item {\tt{p}} に 3 番目の子供がいる
      \item {\tt{p}} の 3 番目の子供の識別子並びに {\tt{x}} がある
      \end{enumerate}
...
\begin{quote}
{\bf 代入判定アルゴリズムにおける特別な場合}
\begin{enumerate}
\item \label{optimize072}
      節点の識別子並びから {\tt{node[t]}} がこの節点と一致している {\tt{t}} を
...
この基本ブロックの出口で生きている変数は {\tt{g, n, 1, 2, 3}} です.
...
1 つ目の{\tt{loff}} をラベルとする節点以外の
{\tt{loff}} をラベルとする節点は 3 番目の子供を持っています.
{\tt{s}} はこの基本ブロックの出口で
生きていないわけですから, 3 番目の子供がいなければ
省略されていたでしょう. しかし, 函数 {\tt{g}} への引数を
{\tt{s}} のアドレスとしている限り, これらは省略できないことが
分かります.

{\bf {\em dag} からのコード生成アルゴリズム} 手順 \ref{optimize064} で
...
図 \ref{optimize090} の {\em dag} において, 3 番地コー
ド {\tt{t5 := a[t4]}} に対応する節点
...
%%\end{latexonly}
\caption{例 \ref{optimize091} の {\em dag}}
\label{optimize096}
\end{center}
\end{figure}
函数 {\tt{f}} は 1 つの基本ブロックから構成されます.
...
{\tt{s}} はこの基本ブロックの出口で生きていないですが
{\tt{g}} の呼び出しの後に {\tt{s[4] := t2}} の節点で
(定義される前に)使用されかつ {\tt{node[s]}} がこの節点と一致している
からコード生成されます.
変換後
\begin{verbatim}
f:
...
それぞれ {\bf 代入判定アルゴリズムにおける特別な場合}に該当
...
に {\bf 代入判定アルゴリズム} を適用すると, この節点は葉なので {\tt{fmt}} が選択
されます. {\tt{p}} はこの後使用される
ので, {\tt{p := fmt}} が {\tt{ap := va\_start fmt}} の先に生成されます. 変換後
\begin{verbatim}
f:
  p := fmt
  ap := va_start fmt
